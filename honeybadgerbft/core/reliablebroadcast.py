...
def encode(K, N, m):
    ...
    Args:
        K (int): K
        N (int): number of blocks to encode string ``m`` into.
        m (str): string to encode.

    Returns:
        list: Erasure codes resulting from encoding ``m`` into ``N``
        blocks using ``zfec`` lib.
    """
...
def decode(K, N, stripes):
    ...
    :param stripes: a container of :math:`N` elements,
...
def merkleTree(strList):
    ...
    Returns
    -------
        list: Merkle tree, represented as a list of ``2*ceil(N)``
            strings. The root digest is at ``tree[1]``, ``tree[0]``
            is blank.
    """
    N = len(strList)
    assert N >= 1
...
def reliablebroadcast(sid, pid, N, f, leader, input, receive, send):
    ...
    leader : int
        0 <= leader < N
    input
        if ``pid == leader``, then :func:`input()` is called to wait
        for the input value
    receive
        :func:`receive()` blocks until a message is received; message is
        of the form::
    ...
    send
        sends (without blocking) a message to a designed recipient
        ``send(i, (tag, ...))``

    Returns
    -------
    str
        ``m`` after receiving :math:`2f+1` ``READY`` messages and
        :math:`N-2f` ``ECHO`` messages
